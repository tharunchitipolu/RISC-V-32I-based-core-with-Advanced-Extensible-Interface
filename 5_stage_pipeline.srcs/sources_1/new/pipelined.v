...
module pipelined #(parameter XLEN = 32)( 
input master_en, clk, rst, fill_random,fill_values
...
begin
 if(rst)
    ...
    begin
    if ((Branch_EX&Branch_out_EX)||Jump_EX)
        begin
        pc_IF <= Jump_Adder_Out_EX; 
        end
    else 
        begin
        pc_IF <= pcplus4_IF;
...
cache#(.ID(1)) I_cache_master_1(                     
                                    .rst(rst), .clk(clk),.read(1'b1),.write(1'b0),.stall(Icache_stall_ID),
                                    ...
                                    .ARLEN(ARLEN_1),.ARREADY(ARREADY),.ARSIZE(ARSIZE_1),.ARVALID(ARVALID_1),
...
if (flush_ID)
    ...
    begin
    Instruction_ID <= I_cache_data_out;
    end
end
...
GPR gpr(.clk(clk), .reset(rst), .wr_en(RegWrite_WR), .rd_en1(1'b1), .rd_en2(1'b1),
    .rd_addr1(rs1_ID), .rd_addr2(rs2_ID), .wr_addr(rd_WR),
    ...
    begin
        if (flush_EX)
          ...
          // gpr signals
           ...
           GPR_rd_data1_EX <=  32'b0 ;
           GPR_rd_data2_EX <=  32'b0 ;
          ...
          // gpr signals
           ...
           GPR_rd_data1_EX <= GPR_rd_data1_ID;
           GPR_rd_data2_EX <=  GPR_rd_data2_ID;
          ...
          //control signals
           ...
           ALUop_EX <=  ALUop_ID;
           Func3_EX <= Func3_ID;
           ALU_select_EX <=  ALU_select_ID;
           Immediate_EX <=   Immediate_ID;
           Branch_out_EX <= Branch_out_ID;
           
           JUMP_Op1Sel_EX <= JUMP_Op1Sel_ID;
           
           Jump_EX <=  Jump_ID;
           Branch_EX <=  Branch_in1_ID;
            end
...
wire [XLEN-1:0] ALU_in1_EX, ALU_in2_EX, ALU_out_EX,GPR_rd_data1_EX_AF,GPR_rd_data2_EX_AF;
...
begin
    if (!stall_MEM)
      ...
      //control signals
       ...
       GPR_rd_data2_MEM <= GPR_rd_data2_EX_AF;
       end
...
cache #(.ID(2)) D_cache_master_2(                     
                                    ...
                                    .ARLEN(ARLEN_2),.ARREADY(ARREADY),.ARSIZE(ARSIZE_2),.ARVALID(ARVALID_2),
...
always@*
begin
PC_sel_EX <= ((Branch_EX&Branch_out_EX)||Jump_EX);
end
...
hazard_unit h1(
     ...
     master_en,
     ForwardAE, ForwardBE ,
     stall_IF, stall_ID, stall_EX, stall_MEM, stall_WR,
     flush_EX, flush_ID);
